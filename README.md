<div align="center">

```
█  █ █████ █     █     ████
█  █ █     █     █     █   █
████ ████  █     █     █   █
█  █ █     █     █     █   █
█  █ █████ █████ █████ ████
██  █   █
█  ██ ██
█  █ █ █
█  █   █
███ █   █
███╗   ███╗ ██████╗ ██╗  ██╗ █████╗ ███╗   ███╗███╗   ███╗███████╗██████╗
████╗ ████║██╔═══██╗██║  ██║██╔══██╗████╗ ████║████╗ ████║██╔════╝██╔══██╗
██╔████╔██║██║   ██║███████║███████║██╔████╔██║██╔████╔██║█████╗  ██║  ██║
██║╚██╔╝██║██║   ██║██╔══██║██╔══██║██║╚██╔╝██║██║╚██╔╝██║██╔══╝  ██║  ██║
██║ ╚═╝ ██║╚██████╔╝██║  ██║██║  ██║██║ ╚═╝ ██║██║ ╚═╝ ██║███████╗██████╔╝
╚═╝     ╚═╝ ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝     ╚═╝╚═╝     ╚═╝╚══════╝╚═════╝ 
```

### `AI Engineer` · `Robotics` · `LLMs` · `Builder`

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/moha22/)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Moh4medehab)
[![Location](https://img.shields.io/badge/Alexandria,_Egypt-FF6B35?style=for-the-badge&logo=googlemaps&logoColor=white)](https://maps.google.com/?q=Alexandria,Egypt)

</div>

---

## `who am i ? `

```python
class MohammedEhab:
    role        = "AI Engineer"
    university  = "Pharos University — CS & AI (2023–2027)"
    location    = "Alexandria, Egypt 🌊"
    languages   = ["Arabic (Native)", "English (C1)"]

    focus = [
        "Large Language Models & RAG Pipelines",
        "Conversational AI & Voice Interfaces",
        "Robotics & Embedded Systems",
        "Human-Robot Interaction",
    ]

    currently_building = "AI systems that actually talk back 🤖🎙️"
```

---

## `My Projects: `

### 🏛️ HORUS — AI-Powered Robotic Museum Guide
> *Founder & Lead Engineer — Jun 2024*

An intelligent robotic guidance system designed to transform how visitors experience museums. HORUS doesn't just answer questions — it **navigates, speaks, listens, and thinks**.

```
STT ──► LangChain RAG ──► LLM ──► TTS ──► Robotic Motion
 🎙️           🧠            💬       🔊           🤖
```

- 🧠 Built full **STT → RAG → LLM → TTS** conversational pipeline from scratch
- 📚 Domain-restricted AI trained on curated museum datasets to minimize hallucinations
- ⚡ Multithreaded architecture synchronizing speech, AI processing & robot motion in real-time
- 🖥️ Custom GUI & multimedia interfaces for seamless human-robot interaction
- 🏆 Presented at **ICMISI 2026** — Smart Innovation Exhibition @ Arab Academy

---

### 🗣️ Museum Conversational Assistant
> *Python · LangChain · RAG*

Voice/chat assistant capable of answering contextual questions about artifacts and exhibition rooms.

- Speech-to-speech pipeline: `mic → STT → LLM+RAG → TTS → speaker`
- LangChain-orchestrated conversational memory & contextual retrieval
- Grounded entirely in museum datasets for reliable, accurate responses

---

### 📚 Book Recommendation System
> *Machine Learning · Python*

ML-powered recommendation engine that suggests books based on user interest patterns — built in Year 1 as an intro to intelligent systems.

---

## `>My Skills :3 `

<div align="center">

| Category | Stack |
|---|---|
| **Languages** | ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white) ![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white) ![C](https://img.shields.io/badge/C-A8B9CC?style=flat-square&logo=c&logoColor=black) ![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=csharp&logoColor=white) ![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=postgresql&logoColor=white) |
| **AI / ML** | ![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat-square&logo=langchain&logoColor=white) ![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white) ![HuggingFace](https://img.shields.io/badge/🤗_Transformers-FFD21E?style=flat-square) ![LangGraph](https://img.shields.io/badge/LangGraph-1C3C3C?style=flat-square) |
| **Databases** | ![ChromaDB](https://img.shields.io/badge/ChromaDB-FF6B35?style=flat-square) ![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white) ![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white) |
| **Robotics** | ![ROS](https://img.shields.io/badge/ROS-22314E?style=flat-square&logo=ros&logoColor=white) ![Arduino](https://img.shields.io/badge/Arduino-00979D?style=flat-square&logo=arduino&logoColor=white) ![Raspberry Pi](https://img.shields.io/badge/Raspberry_Pi-C51A4A?style=flat-square&logo=raspberrypi&logoColor=white) |
| **DevOps/Tools** | ![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white) ![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white) ![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black) |

</div>

---

## `> cat certificates.txt`

```
✅  ROS (Robot Operating System)        — Robotics Corner
✅  Software Engineering                 — Robotics Corner
✅  Arduino Uno                          — Pharos University
✅  Data Science                         — Skills Dynamix
✅  Programming Using Python             — Skills Dynamix
✅  English Proficiency (C1 / Level 9)  — AAST
```

---

## `> history | grep awards`

```
🏆  Certificate of Recognition — ICMISI 2026
    Presented HORUS AI Robotics Project
    Smart Innovation Exhibition
    Arab Academy for Science, Technology & Maritime Transport
```

---

## `> ping me`

<div align="center">

Got an idea for an AI system? Want to talk LLMs, RAG, or robots?  
**Let's build something that thinks.**

[![LinkedIn](https://img.shields.io/badge/Hit_me_up_on_LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/moha22/)
[![Email](https://img.shields.io/badge/+20_1107387020-25D366?style=for-the-badge&logo=whatsapp&logoColor=white)](tel:+201107387020)

---

*"Building AI that doesn't just compute — it communicates."*

![visitors](https://visitor-badge.laobi.icu/badge?page_id=Moh4medehab.Moh4medehab)

</div>
